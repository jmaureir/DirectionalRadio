...
RadioModel::AbstractRadio::AbstractRadio() : rs(this->getId())
{
    radioModel = NULL;
    receptionModel = NULL;
...
void RadioModel::AbstractRadio::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        // read parameters
        transmitterPower = par("transmitterPower");   // initial transmitter power
        ...
        // initialize the reference to the radioChannel
        this->radioChannel=RadioChannel::get();
        if (this->radioChannel==NULL) {
        	error("RadioModel::AbstractRadio requires a RadioChannel acting as ChannelControl");
        }

        // add to the radio module the pathLossAlpha parameter
        // to support the propagation models: they get this parameter form the
        // Radio and not from the channelControl (or RadioChannel)
        this->par("pathLossAlpha") = this->radioChannel->par("alpha");
    ...
    else if (stage == 2)
    {
    	...
    	myHostRef = (RadioChannel::HostEntry*)this->radioChannel->lookupHost(hostModule);

    	// update the host to be sure that the radioChannel knows the initial position of the node
    	// the idea is not depend on basicMobility.
    	Coord initPos = Coord(atoi(myHostRef->host->getDisplayString().getTagArg("p",0)),atoi(myHostRef->host->getDisplayString().getTagArg("p",1)));
		this->radioChannel->updateHostPosition(myHostRef,initPos);

    	//TODO: Add an exception when we have no host

		myHostRef->registerRadio(this);

		this->radioChannel->updateHostChannel(this->myHostRef, rs.getChannelNumber(),this,this->carrierFrequency);

    	// JcM Add: draw the coverage area
    	this->drawCoverageArea();
    }
}

void RadioModel::AbstractRadio::finish()
{
}

RadioModel::AbstractRadio::~AbstractRadio()
{
    delete radioModel;
...
bool RadioModel::AbstractRadio::processAirFrame(AirFrame *airframe)
{

	int chnum = airframe->getChannelNumber();
	AirFrameExtended *airframeext = dynamic_cast<AirFrameExtended *>(airframe);
	if (radioChannel && airframeext)
	{
		double perc = radioChannel->getPercentage();
 ...
 * handleSelfMsg
 */
void RadioModel::AbstractRadio::handleMessage(cMessage *msg)
{
    // handle commands
 ...
 */
void RadioModel::AbstractRadio::bufferMsg(AirFrame *airframe) //FIXME: add explicit simtime_t atTime arg?
{
...
AirFrameExtended *RadioModel::AbstractRadio::encapsulatePacket(cPacket *frame)
{
    ...
    // for directional antennas, PSend will be the nominal transmitter power + directional gain
    airframe->setPSend(transmitterPower);
    EV << "encapsulatePacket: setting PSend = " << transmitterPower << "\n";
...
void RadioModel::AbstractRadio::sendUp(AirFrameExtended *airframe)
{
...
void RadioModel::AbstractRadio::sendDown(AirFrameExtended *airframe)
{
	if (transceiverConnect)
		this->radioChannel->sendToChannel(this, myHostRef, airframe);
	else
		delete airframe;
}
 ...
 */
AirFrameExtended *RadioModel::AbstractRadio::unbufferMsg(cMessage *msg)
{
 ...
 * considered as noise.
 */
void RadioModel::AbstractRadio::handleUpperMsg(AirFrameExtended *airframe)
{
...
void RadioModel::AbstractRadio::handleCommand(int msgkind, cPolymorphic *ctrl)
{
    ...
    else if (msgkind==PHY_C_CHANGETRANSMITTERPOWER)
    {
    	 ...
    	 {
    		 // JcM Fix: Considers the pMax in the radio now
			  transmitterPower = (double) (this->par("pMax"));
    	 }
    }
    else
    {
...
void RadioModel::AbstractRadio::handleSelfMsg(cMessage *msg)
{
    EV<<"RadioModel::AbstractRadio::handleSelfMsg"<<msg->getKind()<<endl;
    ...
    EV<<"RadioModel::AbstractRadio::handleSelfMsg END"<<endl;
}
 ...
 */
void RadioModel::AbstractRadio::handleLowerMsgStart(AirFrame* airframe)
{
    ...
    // if distance is 0 (mutiradio hosts), assume a very small distance
    if (distance == 0) {
    	distance = 0.1; // 10 cm
    }
 ...
 */
void RadioModel::AbstractRadio::handleLowerMsgEnd(AirFrameExtended * airframe)
{
...
void RadioModel::AbstractRadio::addNewSnr()
{
...
void RadioModel::AbstractRadio::changeChannel(int channel)
{
    if (channel == rs.getChannelNumber())
        return;
    if (channel < 0 || channel >= radioChannel->getNumChannels())
    ...
    // update the par to log the channel in the inspectors
    par("channelNumber") = channel;
	...
	RadioChannel::TransmissionList tlAux = radioChannel->getOngoingTransmissions(channel);
	for (RadioChannel::TransmissionList::const_iterator it = tlAux.begin(); it != tlAux.end(); ++it)
	{
	...
	// if this transmission is on our new channel and it would reach us in the future, then schedule it
		...
		if (airframe->getTimestamp() + propagationDelay >= simTime())
		{
			 ...
			 // WRONG: we need to send to each radioIn[] gate of this host
			 // RIGHT: we schedule the packet to be received by the radio
...
void RadioModel::AbstractRadio::setBitrate(double bitrate)
{
...
void RadioModel::AbstractRadio::setRadioState(RadioState::State newState)
{
    rs.setState(newState);
    nb->fireChangeNotification(NF_RADIOSTATE_CHANGED, &rs);
}

void RadioModel::AbstractRadio::updateSensitivity(double rate)
{
...
void RadioModel::AbstractRadio::disconnectReceiver()
{
	myHostRef->unregisterRadio(this);
}

void RadioModel::AbstractRadio::connectReceiver()
{
	myHostRef->registerRadio(this);
	radioChannel->updateHostChannel(myHostRef, rs.getChannelNumber(),this,carrierFrequency);
}

// Paula Uribe: define new method
bool RadioModel::AbstractRadio::isInCoverageArea(Coord& c)
{
    Coord& myPos = (Coord& )getMyPosition();
    double distance = myPos.distance(c);

	double pSend = this->transmitterPower;
	double rcvdPower = receptionModel->calculateReceivedPower(pSend, carrierFrequency, distance);
	double noise = par("thermalNoise");

	// if the receiver power is over the noise, the node is in coverage
...
void RadioModel::AbstractRadio::drawCoverageArea() {
...
double RadioModel::AbstractRadio::getInterferenceDistance(double pRecv) {

    double pSend = par("transmitterPower");
    double pathLossAlpha = this->radioChannel->par("alpha");

    if (pRecv==0) {
    	pRecv = par("thermalNoise");
    }

	double waveLength = (LIGHT_SPEED / carrierFrequency);
	double minReceivePower = pow(10.0, pRecv / 10.0);

	double interfDistance = pow(waveLength * waveLength * pSend /
    						 (16.0 * M_PI * M_PI * minReceivePower), 1.0 / pathLossAlpha);

	//std::cout << "interfDistance " << interfDistance << endl;

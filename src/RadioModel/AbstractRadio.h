...
//
// Extended for Asymmetrical Communications by Paula Uribe and Juan-Carlos Maureira
...
class INET_API AbstractRadio : public BasicModule {
  public:
	AbstractRadio();
  ...
  protected:
	...
	const Coord& getMyPosition() {return this->radioChannel->getHostPosition(myHostRef);}

    virtual int numInitStages() const {return 3;}

  public:
	// Paula Uribe: add new virtual method isInCoverageArea, to be defined in the radio.
	/** Determine if one radio is in the coverage area of the other **/
	virtual bool isInCoverageArea(Coord& c);

	virtual double getInterferenceDistance(double pRecv=0);

  protected:

	RadioChannel* radioChannel;
	RadioChannel::HostEntry* myHostRef;
